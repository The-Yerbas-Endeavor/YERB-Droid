...
inline void keccak_progpow_64(uint32_t* st) noexcept
{
    ...
    static const uint32_t yerbas_kawpow[15] = {
            0x00000072, //R
            0x00000041, //A
            0x00000056, //V
            0x00000045, //E
...
//    ethash_hash256 header_hash = to_hash256(std::string(str_header_hash, 64));
//    ethash_hash256 mix_hash = to_hash256(std::string(str_mix_hash, 64));
//    ethash_hash256 boundary = to_hash256(std::string(str_boundary, 64));
...
//    }
...

    {
        ...
        // 3rd apply yerbas input constraints
        for (int i = 10; i < 25; i++)
            state[i] = yerbas_kawpow[i-10];
