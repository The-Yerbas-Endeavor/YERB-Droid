...
// writes the yerbaswallet address for a scriptPubKey to addr
...
size_t BRAddressFromScriptPubKey(char *addr, size_t addrLen, const uint8_t *script, size_t scriptLen)
{
    ...
    data[0] = YERBAS_PUBKEY_ADDRESS;
#if TESTNET
    data[0] = YERBAS_PUBKEY_ADDRESS_TEST;
#elif REGTEST
    data[0] = YERBAS_PUBKEY_ADDRESS_REGTEST;
#endif
...
#warning TODO: doesn't support PSH count for assets tx will be >3
        ...
        data[0] = YERBAS_SCRIPT_ADDRESS;
#if TESTNET
        data[0] = YERBAS_SCRIPT_ADDRESS_TEST;
#elif REGTEST
        data[0] = YERBAS_SCRIPT_ADDRESS_REGTEST;
#endif
...
// writes the yerbaswallet address for a scriptSig to addr
// returns the number of bytes written, or addrLen needed if addr is NULL
...
{
    static uint8_t pubkeyAddress = YERBAS_PUBKEY_ADDRESS, scriptAddress = YERBAS_SCRIPT_ADDRESS;
    uint8_t data[21];
    size_t r = 0;
...
#if TESTNET
    pubkeyAddress = YERBAS_PUBKEY_ADDRESS_TEST;
    scriptAddress = YERBAS_SCRIPT_ADDRESS_TEST;
#elif REGTEST
    pubkeyAddress = YERBAS_PUBKEY_ADDRESS_REGTEST;
    scriptAddress = YERBAS_SCRIPT_ADDRESS_REGTEST;
#endif
...
// returns true if addr is a valid yerbaswallet address
int BRAddressIsValid(const char *addr)
{
    ...
    if (BRBase58CheckDecode(data, sizeof(data), addr) == 21) {
        r = (data[0] == YERBAS_PUBKEY_ADDRESS || data[0] == YERBAS_SCRIPT_ADDRESS);
    
#if TESTNET
        r = (data[0] == YERBAS_PUBKEY_ADDRESS_TEST || data[0] == YERBAS_SCRIPT_ADDRESS_TEST);
#elif REGTEST
        r = (data[0] == YERBAS_PUBKEY_ADDRESS_REGTEST || data[0] == YERBAS_SCRIPT_ADDRESS_REGTEST);
#endif
    }
