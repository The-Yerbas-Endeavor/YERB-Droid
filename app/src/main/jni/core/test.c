...
//        r = 0, fprintf(stderr, "***FAILED*** %s: WalletRegisterTransaction() test 1\n", __func__);
    ...
    amt = YerbasAmount(50000, 50000);
    if (amt != CORBIES) r = 0, fprintf(stderr, "***FAILED*** %s: YerbasAmount() test 1\n", __func__);

    amt = YerbasAmount(-50000, 50000);
    if (amt != -CORBIES) r = 0, fprintf(stderr, "***FAILED*** %s: YerbasAmount() test 2\n", __func__);
