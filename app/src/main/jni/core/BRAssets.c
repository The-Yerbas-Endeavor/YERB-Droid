...
bool IsNewAsset(const BRTransaction *tx) {
    ...
    // Issuing an Asset must contain at least 1 TxOut( Yerbas Burn Output, Any Number of other Outputs ..., Owner Asset Output, issue Output)
    if (tx->outCount < 3)
        return false;
...
bool IsReissueAsset(const BRTransaction *tx) {
    // Reissuing an Asset must contain at least 3 CTxOut ( Yerbas Burn Tx, Any Number of other Outputs ..., Reissue Asset Output, Owner Asset Change Output)
    if (tx->outCount < 3)
        return false;
...
bool CheckReissueBurnTx(const BRTxOutput *txOut) {
    // Check the transaction and verify that the correct YERB Amount
