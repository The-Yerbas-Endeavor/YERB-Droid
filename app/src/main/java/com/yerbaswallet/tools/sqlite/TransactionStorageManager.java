...
public class TransactionStorageManager {
    ...
    public static boolean putTransaction(Context app, String iso, BRTransactionEntity tx) {
        ...
        if (iso.equalsIgnoreCase("YERB")/* || iso.equalsIgnoreCase("bch")*/) {
            BRTransactionEntity result = YerbTransactionDataStore.getInstance(app).putTransaction(app, iso, tx);
            return result != null;
        }
    ...
    public static List<BRTransactionEntity> getTransactions(Context app, String iso) {
        ...
        if (iso.equalsIgnoreCase("YERB")/* || iso.equalsIgnoreCase("bch")*/) {
            return YerbTransactionDataStore.getInstance(app).getAllTransactions(app, iso);
        }
    ...
    public static boolean updateTransaction(Context app, String iso, BRTransactionEntity tx) {
        ...
        if (iso.equalsIgnoreCase("YERB")/* || iso.equalsIgnoreCase("bch")*/) {
            return YerbTransactionDataStore.getInstance(app).updateTransaction(app, iso, tx);
    ...
    public static boolean removeTransaction(Context app, String iso, String hash) {
        ...
        if (iso.equalsIgnoreCase("YERB")/* || iso.equalsIgnoreCase("bch")*/) {
            YerbTransactionDataStore.getInstance(app).deleteTxByHash(app, iso, hash);
            return true;
        }
        //other wallets
