...
import com.yerbaswallet.core.BRCoreChainParams;
import com.yerbaswallet.core.BRCoreKey;
import com.yerbaswallet.core.BRCoreMasterPubKey;
import com.yerbaswallet.core.BRCoreMerkleBlock;
import com.yerbaswallet.core.BRCorePeer;
import com.yerbaswallet.core.BRCorePeerManager;
import com.yerbaswallet.core.BRCoreTransaction;
import com.yerbaswallet.core.BRCoreTransactionAsset;
import com.yerbaswallet.core.BRCoreWallet;
import com.yerbaswallet.core.BRCoreWalletManager;
import com.yerbaswallet.core.MyTransactionAsset;
import com.yerbaswallet.presenter.customviews.BRToast;
import com.yerbaswallet.presenter.entities.BRMerkleBlockEntity;
import com.yerbaswallet.presenter.entities.BRPeerEntity;
import com.yerbaswallet.presenter.entities.BRTransactionEntity;
import com.yerbaswallet.presenter.entities.BlockEntity;
import com.yerbaswallet.presenter.entities.CurrencyEntity;
import com.yerbaswallet.presenter.entities.PeerEntity;
import com.yerbaswallet.presenter.entities.TxUiHolder;
import com.yerbaswallet.presenter.fragments.FragmentPin;
import com.yerbaswallet.presenter.interfaces.BRAuthCompletion;
import com.yerbaswallet.presenter.interfaces.BROnSignalCompletion;
import com.yerbaswallet.presenter.interfaces.ConfirmationListener;
import com.yerbaswallet.presenter.interfaces.WalletManagerListener;
...
import com.yerbaswallet.tools.manager.BRNotificationManager;
...
import com.yerbaswallet.tools.sqlite.MerkleBlockDataSource;
import com.yerbaswallet.tools.sqlite.PeerDataSource;
import com.yerbaswallet.tools.sqlite.YerbTransactionDataStore;
import com.yerbaswallet.tools.sqlite.TransactionStorageManager;
...
import com.yerbaswallet.tools.util.SymbolUtils;
...
import com.yerbaswallet.wallet.abstracts.OnTxListModified;
import com.yerbaswallet.wallet.abstracts.OnTxStatusUpdatedListener;
import com.yerbaswallet.wallet.abstracts.SyncListener;
import com.yerbaswallet.wallet.configs.WalletUiConfiguration;
...
import static com.yerbaswallet.tools.util.BRConstants.CREATION_FEE;
import static com.yerbaswallet.tools.util.BRConstants.REISSUE_FEE;
import static com.yerbaswallet.tools.util.BRConstants.ROUNDING_MODE;
import static com.yerbaswallet.tools.util.BRConstants.SATOSHIS;
import static com.yerbaswallet.tools.util.BRConstants.SUB_FEE;
import static com.yerbaswallet.tools.util.BRConstants.UNIQUE_FEE;

public class YerbWalletManager extends BRCoreWalletManager implements BaseWalletManager {

    private static final String TAG = YerbWalletManager.class.getName();

    public static String ISO = "YERB";

    private static final String mName = "Yerbas";
    public static final String YERB_SCHEME = "yerbas";
    public static final long MAX_YERB = 21000000 * 1000L;

    private static YerbWalletManager instance;
    ...
    public synchronized static YerbWalletManager getInstance(Context app) {
        if (instance == null) {
...
//            int time = (int) (System.currentTimeMillis() / DateUtils.SECOND_IN_MILLIS);
            ...
            instance = new YerbWalletManager(app, pubKey, BuildConfig.TESTNET ? BRCoreChainParams.testnetChainParams : BRCoreChainParams.mainnetChainParams, time);
        }
        return instance;
    }

    private YerbWalletManager(final Context app, BRCoreMasterPubKey masterPubKey,
...
//        String jsonString = /*BRApiManager.urlGET(app, "https://" + YerbasApp.HOST + "/fee-per-kb?currency=" + getIso(app));*/
    ...
    public String getSymbol(Context app) {
        ...
        if (app != null) {
            ...
            switch (unit) {
                case BRConstants.CURRENT_UNIT_UYERB:
                    if (symbolUtils.doesDeviceSupportSymbol(BRConstants.symbolYerbasPrimary)) {
                        currencySymbolString = "µ" + BRConstants.symbolYerbasPrimary;
                    } else {
                        currencySymbolString = "µ" + BRConstants.symbolYerbasSecondary;
                    }
                    break;
                case BRConstants.CURRENT_UNIT_MYERB:
                    if (symbolUtils.doesDeviceSupportSymbol(BRConstants.symbolYerbasPrimary)) {
                        currencySymbolString = "m" + BRConstants.symbolYerbasPrimary;
                    } else {
                        currencySymbolString = "m" + BRConstants.symbolYerbasSecondary;
                    }
                    break;
                case BRConstants.CURRENT_UNIT_YERBASS:

                    if (symbolUtils.doesDeviceSupportSymbol(BRConstants.symbolYerbasPrimary)) {
                        currencySymbolString = BRConstants.symbolYerbasPrimary;
    ...
    public void wipeData(Context app) {
        YerbTransactionDataStore.getInstance(app).deleteAllTransactions(app, getIso(app));
...
//        clearAppData(app);
    ...
    public BigDecimal getMaxAmount(Context app) {
        //return max yerbas
        return new BigDecimal(MAX_YERB);
    }
    ...
    public void txPublished(final String error) {
        ...
        final Context app = YerbasApp.getYerbContext();
      /*  if (Utils.isNullOrEmpty(error)) return;
        BRExecutor.getInstance().forMainThreadTasks().execute(new Runnable() {
            @Override
            public void run() {
                if (app instanceof Activity)
                    BRAnimator.showYerbSignal((Activity) app, app.getString(R.string.Alert_error),
        ...
        BRExecutor.getInstance().forMainThreadTasks().execute(new Runnable() {
            ...
            public void run() {
                if (app instanceof Activity) {
                    BRAnimator.showYerbSignal((Activity) app, alertTitle,
    ...
    public void balanceChanged(long balance) {
        ...
        Context app = YerbasApp.getYerbContext();
    ...
    public void txStatusUpdate() {
        ...
        BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
            ...
            public void run() {
                ...
                final Context ctx = YerbasApp.getYerbContext();
                if (ctx == null) return;
    ...
    public BRCoreTransaction[] loadTransactions() {
        ...
        List<BRTransactionEntity> txs = YerbTransactionDataStore.getInstance(app).getAllTransactions(app, getIso(app));
...
//        KVStoreManager.getInstance().putTxMetaData(ctx, metaData, transaction.getHash());
                            ...
                            if (!BRToast.isToastShown()) {
                                if (Utils.isEmulatorOrDebug(ctx))
                                    BRToast.showCustomToast(ctx, strToShow,
                                            YerbasApp.DISPLAY_HEIGHT_PX / 2, Toast.LENGTH_LONG, R.drawable.toast_layout_black);
    ...
    public void onTxUpdated(final String hash, int blockHeight, int timeStamp) {
        ...
        Context ctx = YerbasApp.getYerbContext();
        if (ctx != null) {
