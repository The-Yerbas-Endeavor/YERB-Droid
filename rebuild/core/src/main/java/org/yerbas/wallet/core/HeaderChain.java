...
public final class HeaderChain {
    ...
    private final ProofOfWorkVerifier proofOfWork;

    public HeaderChain(HeaderStore store) {
        this(store, ProofOfWorkVerifier.unavailable());
    }

    public HeaderChain(HeaderStore store, ProofOfWorkVerifier proofOfWork) {
        this.store = Objects.requireNonNull(store);
        this.proofOfWork = Objects.requireNonNull(proofOfWork);
    }
    ...
    public HeaderStore.StoredHeader connect(BlockHeader header) {
        ...
        if (height >= DarkGravityWave.WINDOW) {
            List<BlockHeader> history = newestFirstHistory(parent, DarkGravityWave.WINDOW);
            long expected = DarkGravityWave.nextTarget(history);
            if (header.compactTarget() != expected) {
                throw new IllegalArgumentException("invalid DGW target at height " + height);
            }
        }

        if (!proofOfWork.verify(header)) {
            throw new IllegalArgumentException("invalid or unavailable GhostRider proof of work");
        }
    ...
    private List<BlockHeader> newestFirstHistory(HeaderStore.StoredHeader start, int count) {
        List<BlockHeader> result = new ArrayList<>(count);
        HeaderStore.StoredHeader cursor = start;
        while (result.size() < count) {
            result.add(cursor.header());
            if (cursor.height() == 0) break;
            cursor = store.find(cursor.header().previousBlock())
                    .orElseThrow(() -> new IllegalStateException("missing ancestor for DGW validation"));
        }
        if (result.size() < count) {
            throw new IllegalStateException("insufficient DGW history");
        }
